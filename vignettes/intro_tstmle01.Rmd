...
The `tstmle01` package implements the targeted maximum likelihood estimation (TMLE) of causal
effects based on the observation of a single binary time-series. Current implementation supports iterative TMLE; check back for one-step and online TMLE. For the (faster) package implementing a more general methodology and other time-series based target parameters, see [tstmle](https://github.com/podTockom/tstmle/). 
