...
#' addition, it returns the variance of the estimate and confidence intervals.
#'
...
#' @param intervention1 Specify g^*, of P(A|past). Right now supports only 1/0
#'  type interventions.
#' @param intervention2 Specify g^* to compate to. Right now supports only 1/0
#'  type interventions.
...
tstmle01 <- function(data, freqY, freqA, freqW, t, Anode, intervention1,
  ...
  est1 <- mainTMLE(
    fit, t = t, Anode = Anode, intervention = intervention1,
    B = B, N = N, MC = MC, maxIter = maxIter, tol = tol
  )

  est2 <- mainTMLE(
    fit, t = t, Anode = Anode, intervention = intervention2,
    B = B, N = N, MC = MC, maxIter = maxIter, tol = tol
  )
